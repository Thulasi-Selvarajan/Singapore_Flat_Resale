...
def extract_tar(tar_file_path, extract_to_path):
    with tarfile.open(tar_file_path, 'r:gz') as tar:  # Specify 'r:gz' for reading gzipped TAR files
        ...
        print("Extracted files to:", extract_to_path)
        print("List of extracted files:", tar.getnames())
...
print("CSV file path:", extracted_csv_path)
...
with tab2:
    ...
    if option == 'Prediction Tab':    
        ...
        with col5:
            ...
            remaining_lease = st.text_input("**Enter the remaining lease duration (YY-M)**")
...
with tab3:
    ...
    if st.button('Predict and Conclusion'):
            ...
            st.markdown('#### :violet["Factors Influencing Resale Prices of Flats: A Quick Comprehensive Analysis"]')
